...
const DEFAULT_IMAGES: ImageItem[] = [
  ...
  {
    src: "/pics/IMG_8035.jpg", 
    alt: "Más comida polaca",
  },
  {
    src: "/pics/IMG_8294.jpg",
    alt: "El viaje más largo",
  },
  {
...
export default function DomeGallery({
  ...
  // Auto-rotation logic - slow continuous rotation
  useEffect(() => {
    const startAutoRotation = () => {
      let lastTime = 0;
      const rotationSpeed = 0.03; // Very slow rotation - degrees per frame
      
      const step = (currentTime: number) => {
        if (!draggingRef.current && !focusedElRef.current && !inertiaRAF.current) {
          if (lastTime !== 0) {
            const deltaTime = currentTime - lastTime;
            const rotationIncrement = (rotationSpeed * deltaTime) / 16.67; // normalize to ~60fps
            
            rotationRef.current.y = wrapAngleSigned(rotationRef.current.y + rotationIncrement);
  ...
  // Manual drag handling without @use-gesture/react
  useEffect(() => {
    ...
    return () => {
      ...
      main.removeEventListener('pointercancel', handlePointerUp);
    };
  }, [dragSensitivity, maxVerticalRotationDeg, stopInertia]);
